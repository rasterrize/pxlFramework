...
#define BASS_CHECK(func) if (!func) { PXL_LOG_ERROR(LogArea::Audio, "BASS audio error: {} - {}", BASS_ErrorGetCode(), pxl::AudioUtil::ErrorCodeToString(BASS_ErrorGetCode())); }

namespace pxl
{
    namespace AudioUtil
    {
        inline std::string ErrorCodeToString(int code)
        {
            switch (code)
            {
                ...
                case 500: return "BASS_ERROR_JAVA_CLASS";
